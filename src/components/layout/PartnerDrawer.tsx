...
export function PartnerDrawer({ isOpen, onClose }: PartnerDrawerProps) {
  ...
  async function onSubmit(values: Record<string, string>) {
    ...
    await submitInterest("partner_inquiries", {
      ...values,
      phone,
  ...
  return (
    <AnimatePresence>
      {isOpen ? (
        ...
        >
          <motion.aside
            className="ml-auto flex h-full w-full max-w-[430px] flex-col overflow-y-auto bg-white p-6 shadow-2xl"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <button onClick={onClose} aria-label="Close partner drawer" className="rounded-full p-2 hover:bg-asel-off-white">
                <X />
              </button>
            </div>
            <p className="mt-5 text-asel-mid-gray">
              We welcome institutions, companies, policymakers, investors, and development partners committed to sustainable energy and inclusive innovation.
            </p>
            ...
            {submitted ? (
              ...
              <form className="mt-8 grid gap-4" onSubmit={handleSubmit(onSubmit)}>
                <DrawerField label="Organisation Name">
                  <input required {...register("organisation_name")} />
                </DrawerField>
                <DrawerField label="Partnership Type">
                  <select {...register("partnership_type")}>
                    <option>Training Delivery</option>
                    <option>Funding/Grant</option>
                ...
                <DrawerField label="Contact Name">
                  <input required {...register("contact_name")} />
                </DrawerField>
                <DrawerField label="Email">
                  <input type="email" required {...register("email")} />
                </DrawerField>
                <div className="grid gap-2 text-sm font-bold text-asel-navy">
                  <span>Phone</span>
                  <PhoneInput value={phone} onChange={setPhone} />
                </div>
                <DrawerField label="Brief description">
                  <textarea rows={5} required {...register("message")} />
                </DrawerField>
            ...
            <p className="mt-6 text-sm text-asel-mid-gray">
              Or email us directly at{" "}
              <span className="font-bold text-asel-navy">info@aselafrica.org</span>
            </p>
          </motion.aside>
        </motion.div>
      ) : null}
